...
function CardForm({ initial, onSave, onCancel }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <input required placeholder="Nome do cartão" className="w-full border border-slate-200 dark:border-slate-600 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white dark:bg-slate-700 text-slate-800 dark:text-slate-100"
        value={form.name} onChange={e => set('name', e.target.value)} />
      <input placeholder="Banco" className="w-full border border-slate-200 dark:border-slate-600 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white dark:bg-slate-700 text-slate-800 dark:text-slate-100"
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="text-xs text-slate-500 dark:text-slate-400 mb-1 block">Dia de fechamento</label>
          <input required type="number" min={1} max={31} className="w-full border border-slate-200 dark:border-slate-600 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white dark:bg-slate-700 text-slate-800 dark:text-slate-100"
            value={form.closing_day} onChange={e => set('closing_day', parseInt(e.target.value))} />
        </div>
        <div>
          <label className="text-xs text-slate-500 dark:text-slate-400 mb-1 block">Dia de vencimento</label>
          <input required type="number" min={1} max={31} className="w-full border border-slate-200 dark:border-slate-600 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white dark:bg-slate-700 text-slate-800 dark:text-slate-100"
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-slate-800 dark:text-slate-100">Cartões de Crédito</h1>
      ...
      {cards.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {cards.map(c => (
            <div key={c.id} className={`bg-white dark:bg-slate-800 rounded-2xl border p-5 ${c.is_default ? 'border-amber-200' : 'border-slate-100 dark:border-slate-700'}`}>
